...
// Services Page - Animated grid with organic wave pattern from bottom to top
export const InteractiveGridAnimatedVertical: React.FC = () => {
  ...
  // Animated wave with organic, non-uniform shape - vertical movement (bottom to top)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    setCells((prev) =>
      prev.map((cell) => {
        ...
        const verticalPosition = cell.y / gridSize;
        // Start from bottom (high values) and move to top (low values)
        const maxVerticalPosition = Math.max(...prev.map((c) => c.y / gridSize));
        const wavePosition = maxVerticalPosition - animationOffset / 2;
        ...
        const distance = Math.abs(verticalPosition - organicWavePosition);

...
export async function setOneShotSession(file: File, token: string, draftType?: string): Promise<void> {
    const db = await openDB();
    return new Promise((resolve, reject) => {
        const transaction = db.transaction(STORE_NAME, 'readwrite');
        const store = transaction.objectStore(STORE_NAME);
        // We use a fixed key because we only support ONE active session at a time (Sovereignty)
        const request = store.put({ key: 'active_session', file, token, draftType, timestamp: Date.now() });
...
export async function consumeOneShotSession(): Promise<{ file: File; token: string; draftType?: string } | null> {
    const db = await openDB();
    return new Promise((resolve, reject) => {
        const transaction = db.transaction(STORE_NAME, 'readwrite');
        const store = transaction.objectStore(STORE_NAME);
